...
MeshResult tessellate_parallel(
    const OcctShape& shape,
    double deflection,
    int32_t num_threads  // 0 = auto-detect
);

/// Generate multiple LOD meshes in parallel
/// Note: LODMeshResult struct is generated by cxx from Rust FFI
LODMeshResult generate_lods_parallel(
    const OcctShape& shape,
