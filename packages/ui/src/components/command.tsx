...
} from "@cadhy/ui/components/autocomplete"
...
function Command({
  ...
  className,
  ...props
}: React.ComponentProps<typeof Autocomplete> & { className?: string }) {
  return (
...
function CommandPanel({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      className={cn(
        "-mx-px relative flex min-h-0 flex-1 flex-col overflow-hidden rounded-t-xl border bg-popover bg-clip-padding shadow-xs [clip-path:inset(0_1px)] before:pointer-events-none before:absolute before:inset-0 before:rounded-t-[calc(var(--radius-xl)-1px)] **:data-[slot=scroll-area-scrollbar]:mt-2 dark:bg-clip-border dark:before:shadow-[0_-1px_--theme(--color-white/8%)]",
        className
      )}
